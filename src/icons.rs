...
use crate::stylesheet::styles::Theme;

const ICON_FONT: iced::Font = iced::Font::with_name("QuantumLauncher");

pub fn icon<'a>(codepoint: char) -> iced::widget::Text<'a, Theme> {
    iced::widget::text(codepoint).font(ICON_FONT)
}

pub fn icon_with_size<'a>(codepoint: char, size: u32) -> iced::widget::Text<'a, Theme> {
...
macro_rules! icon_define {
    ($name:ident, $unicode:expr) => {
        paste! {
            #[allow(dead_code)]
            pub fn $name<'a>() -> iced::widget::Text<'a, Theme> {
                icon($unicode)
            }

            #[allow(dead_code)]
            pub fn [<$name _s>]<'a>(size: u32) -> iced::widget::Text<'a, Theme> {
